...
int main(int argc, char *argv[]){
  if(argc == 3){
	...
	sem_destroy(&locks[i]);
      ...
      for(int i = 0; i < G; i++){
	pthread_join(*(thread_of_groups + i), NULL);
      }
  ...
  for(int i = 0; i < num_of_houses; i++){
	...
	if(houses_in_neighborhood[i].numOfCandy >= g.numOfKids){
	  if(manhattan_distance < closest_distance){
	    ...
	    closest = i;
	    first = 0;
	  }// end of checking if distance is shorter
	}//end of checking if there's enough candy for the group
      }//end of checking if this is the first valid house
    } //end of checking if house is starting or current house
  }//end of for loop
